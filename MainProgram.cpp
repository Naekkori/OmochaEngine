...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (arg == "--setVsync" && i + 1 < argc) // Use else if to avoid re-checking if it was --setfps
        {
            try {
                ...
                cerr << "Warning: Invalid argument for --setVsync. Expected a number (0 or 1). Using default (1)." <<
                        endl;
            ...
            try {
                ...
                cerr << "Warning: Invalid argument for --showfps. Expected a number (0 or 1). Using default (0)." <<
                        endl;
                engine.specialConfig.showFPS = false;
            }
            catch (const out_of_range &) {
    ...
    if (filesystem::exists(insideprojectPath)) {
        ...
        engine.showMessageBox("project.json not found!\nPlease place the project file in the 'assets' folder.",
                              engine.msgBoxIconType.ICON_ERROR);
    ...
    if (projectDataLoaded) {
        if (!engine.initGE(mainProgram.mainProgramValue.getVsync(), mainProgram.mainProgramValue.useVulkan))
        // VSync 및 Vulkan 사용 여부 전달
        {
