...
Engine::Engine() : window(nullptr), renderer(nullptr),
                   ...
                   zoomFactor((this->specialConfig.setZoomfactor <= 0.0)
                                  ...
                                  : std::clamp(static_cast<float>(this->specialConfig.setZoomfactor), Engine::MIN_ZOOM,
                                               Engine::MAX_ZOOM)), m_isDraggingZoomSlider(false), m_pressedObjectId(""),
...
string Engine::getSafeStringFromJson(const rapidjson::Value &parentValue,
    ...
    if (!parentValue.IsObject()) {
        EngineStdOut(
            ...
            RapidJsonValueToString(parentValue), 2);
        return defaultValue;
    }
    ...
    if (s_val.empty() && !allowEmpty) {
        if (isCritical) {
            EngineStdOut(
                ...
                " is an empty string, but empty is not allowed. Using default: '" + defaultValue + "'.", 2);
        } else {
            EngineStdOut(
                "Field '" + fieldName + "' in " + contextDescription +
                " is an empty string, but empty is not allowed. Using default: '" + defaultValue + "'.", 1);
        }
        return defaultValue;
    }
...
bool Engine::loadProject(const string &projectFilePath) {
    ...
    if (document.HasParseError()) {
        string errorMsg = string("Failed to parse project file: ") + rapidjson::GetParseError_En(
        ...
        for (rapidjson::SizeType i = 0; i < variablesJson.Size(); ++i) {
            ...
            if (!variableJson.IsObject()) {
                EngineStdOut(
                    ...
                    RapidJsonValueToString(variableJson), 3);
                continue;
            }
            ...
            if (variableJson.HasMember("value")) {
                ...
                if (valNode.IsString()) {
                    ...
                    EngineStdOut(
                        ...
                        RapidJsonValueToString(valNode), 1);
                }
            } else {
            ...
            if (currentVarDisplay.value.empty()) {
                EngineStdOut(
                    ...
                    to_string(i) + ". Skipping variable.", 1);
                continue;
            }
                    ...
                    for (rapidjson::SizeType j = 0; j < arrayJson.Size(); ++j) {
                        ...
                        if (!itemJson.IsObject()) {
                            EngineStdOut(
                                ...
                                "' is not an object. Skipping. Content: " + RapidJsonValueToString(itemJson), 1);
                            continue;
                        }
                        ListItem item;
                        item.key = getSafeStringFromJson(itemJson, "key",
                                                         "list item entry " + to_string(j) + " for " + currentVarDisplay
                                                         .name, "", false, true); // key는 빈 문자열 허용
                        item.data = getSafeStringFromJson(itemJson, "data",
                                                          "list item entry " + to_string(j) + " for " +
                                                          currentVarDisplay.name, "", true,
        ...
        for (rapidjson::SizeType i = 0; i < objectsJson.Size(); ++i) {
            ...
            if (!objectJson.IsObject()) {
                EngineStdOut(
                    ...
                    RapidJsonValueToString(objectJson), 1);
                continue;
            }
            ...
            if (objectJson.HasMember("sprite") && objectJson["sprite"].IsObject() &&
                ...
                for (rapidjson::SizeType j = 0; j < soundsJson.Size(); ++j) {
                    ...
                    if (soundJson.IsObject() && soundJson.HasMember("id") && soundJson["id"].IsString() && soundJson.
                        HasMember("filename") && soundJson["filename"].IsString()) {
                        SoundFile sound;
                        ...
                        if (soundJson.HasMember("duration") && soundJson["duration"].IsNumber()) {
                            ...
                            EngineStdOut(
                                ...
                                ") is missing 'duration' or it's not numeric. Using default duration 0.0.", 1);
                        }
            ...
            if (!selectedCostumeFound && objectJson.HasMember("selectedCostume") && objectJson["selectedCostume"].
                IsString()) {
            ...
            if (selectedCostumeFound) {
                ...
                if (!objInfo.costumes.empty()) {
                    ...
                    EngineStdOut(
                        ...
                        objInfo.costumes[0].id, 1);
                } else {
                    EngineStdOut(
                        "Object '" + objInfo.name + "' (ID: " + objInfo.id +
                        ") is missing selectedPictureId/selectedCostume and has no costumes.", 1);
                    ...
                    if (entityJson.HasMember("colour") && entityJson["colour"].IsString()) {
                        ...
                        if (hexColor.length() == 7 && hexColor[0] == '#') {
                            ...
                            EngineStdOut(
                                ...
                                hexColor + ". Using default #000000.", 1);
                            objInfo.textColor = {0, 0, 0, 255};
                        }
                    } else {
                        EngineStdOut(
                            "textBox '" + objInfo.name +
                            "' is missing 'colour' field or it's not a string. Using default #000000.", 1);
                        objInfo.textColor = {0, 0, 0, 255};
                    }
                    ...
                    if (entityJson.HasMember("font") && entityJson["font"].IsString()) {
                        ...
                        if (pxPos != string::npos) {
                            try {
                                ...
                                EngineStdOut(
                                    "INFO: textBox '" + objInfo.name + "' font size parsed: " + to_string(
                                        objInfo.fontSize), 0);
                            } catch (...) {
                                objInfo.fontSize = 20;
                                EngineStdOut(
                                    "Failed to parse font size from '" + fontString + "' for textBox '" + objInfo.name +
                                    "'. Using default size 20.", 1);
                            }
                            ...
                            EngineStdOut(
                                "INFO: textBox '" + objInfo.name + "' font name parsed: '" + objInfo.fontName + "'", 0);
                        } else {
                            objInfo.fontSize = 20;
                            objInfo.fontName = fontString;
                            EngineStdOut(
                                "textBox '" + objInfo.name + "' 'font' field is not in 'size px Name' format: '" +
                                fontString + "'. Using default size 20 and '" + objInfo.fontName + "' as name.", 1);
                        }
                    } else {
                        EngineStdOut(
                            "textBox '" + objInfo.name +
                    ...
                    if (entityJson.HasMember("textAlign") && entityJson["textAlign"].IsNumber()) {
                        ...
                        if (parsedAlign >= 0 && parsedAlign <= 2) {
                            ...
                            EngineStdOut(
                                ...
                                to_string(parsedAlign) + ". Using default alignment 0 (left).", 1);
                        }
                        EngineStdOut(
                        ...
                        EngineStdOut(
                            "textBox '" + objInfo.name +
                            "' is missing 'textAlign' field or it's not numeric. Using default alignment 0.", 1);
                    }
                } else {
                    EngineStdOut(
                        "textBox '" + objInfo.name +
                        "' is missing 'entity' block or it's not an object. Cannot load text box properties.", 1);
            ...
            if (objectJson.HasMember("entity") && objectJson["entity"].IsObject()) {
                ...
                EngineStdOut(
                    ...
                    ") is missing 'entity' block or it's not an object. Cannot create Entity.", 1);
            }
            /**
             * @brief 스크립트 블럭
             *
            ...
            if (objectJson.HasMember("script") && objectJson["script"].IsString()) {
                ...
                if (scriptString.empty()) {
                    EngineStdOut(
                        ...
                        "' has an empty 'script' string. No scripts will be loaded for this object.", 0);
                } else {
                        ...
                        if (scriptDocument.IsArray()) {
                            ...
                            for (rapidjson::SizeType j = 0; j < scriptDocument.Size(); ++j) {
                                ...
                                if (scriptStackJson.IsArray()) {
                                    ...
                                    EngineStdOut(
                                        "  Parsing script stack " + to_string(j + 1) + "/" + to_string(
                                    ...
                                    EngineStdOut(
                                        ...
                                        "' is not an array of blocks (not a valid script stack). Skipping this script stack. Content: "
                                        + RapidJsonValueToString(scriptStackJson), 1);
                                }
                            }
                            objectScripts[objectId] = scriptsForObject;
                            EngineStdOut(
                                "INFO: Parsed " + to_string(scriptsForObject.size()) + " script stacks for object ID: "
                                + objectId, 0);
                        } else {
                            EngineStdOut(
                                ...
                                + RapidJsonValueToString(scriptDocument), 1);
                        }
                    } else {
                ...
                EngineStdOut(
                    "INFO: Object '" + objInfo.name +
                    "' is missing 'script' field or it's not a string. No scripts will be loaded for this object.", 0);
            }
        }
    } else {
        ...
        for (rapidjson::SizeType i = 0; i < scenesJson.Size(); ++i) {
            ...
            if (!sceneJson.IsObject()) {
                EngineStdOut(
                    ...
                    RapidJsonValueToString(sceneJson), 1);
                continue;
            }

            if (sceneJson.HasMember("id") && sceneJson["id"].IsString() && sceneJson.HasMember("name") && sceneJson[
                    "name"].IsString()) {
                ...
                if (firstBlock.type == "when_run_button_click") {
                    ...
                    if (script.blocks.size() > 1) {
                        ...
                        if (firstBlock.paramsJson.IsArray() && firstBlock.paramsJson.Size() > 0) {
                            ...
                            if (keyIdentifierFound) {
                                ...
                                EngineStdOut(
                                    " -> object ID " + objectId +
                                    " 'press key' invalid param or missing message ID. Params JSON: " + string(
                                        buffer.GetString()) + ".", 1);
                            }
                        }
                    }
                    ...
                    if (script.blocks.size() > 1) {
                        ...
                        if (messageParamFound && !messageIdToReceive.empty()) {
                            ...
                            EngineStdOut(
                                " -> object ID " + objectId +
                                " 'recive signal' invalid param or missing message ID. Params JSON: " + string(
                                    buffer.GetString()) + ".", 1);
                        }
                    }
    ...
    if (renderer && !m_HUDVariables.empty()) {
        ...
        if (windowW > 0 && windowH > 0) {
            ...
            EngineStdOut(
                ...
                ") not valid for initial HUD clamping.", 1);
        }
    }
    return true;
}
                ...
                if (!costume.imageHandle) // 텍스처 로드 실패 시 렌더러 포인터 값 로깅
                {
                    EngineStdOut(
                        "Renderer pointer value at IMG_LoadTexture failure: " + to_string(
                            reinterpret_cast<uintptr_t>(this->renderer)), 3);
                }
                if (costume.imageHandle) {
                    loadedCount++;
                    EngineStdOut(
...
bool Engine::loadSounds() {
    ...
    EngineStdOut(
        ...
        to_string(loadingDuration.count()) + " seconds.", 0);
    return true;
}
                ...
                if (!determinedFontPath.empty()) {
                    ...
                    if (!Usefont) {
                        EngineStdOut(
                            ...
                            " for textBox '" + objInfo.name + "'. Falling back to HUD font.", 2);
                        Usefont = hudFont;
                    }
                } else {
                            ...
                            if (currentEntity->isVisible()) // 엔티티가 보이는지 확인
                            {
                                ...
                                if (objInfoPtr) {
                                    ...
                                    EngineStdOut(
                                        ...
                                        "' during mouse_clicked event processing. Script not run.", 1);
                                }
                            }
                        }
                    }
                    ...
                    for (const auto &scriptPair: m_mouseClickCanceledScripts) {
                        ...
                        if (currentEntity && currentEntity->isVisible()) {
                            ...
                            if (objInfoPtr) {
                                ...
                                EngineStdOut(
                                    "Warning: ObjectInfo not found for entity ID '" + objectId +
                                    "' during mouse_click_canceled event processing. Script not run.", 1);
                            }
...
void Engine::goToNextScene() {
    ...
    if (it != m_sceneOrder.end()) {
        ...
        EngineStdOut(
            ...
            "' not found in defined scene order. Cannot determine next scene.", 2);
    }
}
...
void Engine::goToPreviousScene() {
    ...
    if (it != m_sceneOrder.end()) {
        ...
        EngineStdOut(
            "Error: Current scene ID '" + currentSceneId +
            "' not found in defined scene order. Cannot determine previous scene.", 2);
    }
}
...
void Engine::changeObjectIndex(const std::string &entityId, Omocha::ObjectIndexChangeType changeType) {
    ...
    EngineStdOut(
        ...
        " to new index " + std::to_string(targetIndex) + " (0 is topmost).", 0);
}
        ...
        if (objInfo.id == entityId) {
            if (objInfo.costumes.size() <= 1) {
                ...
                EngineStdOut(
                    ...
                    " costume(s). Cannot change to next/previous.", 1);
                return false;
            }
            ...
            if (currentCostumeIndex == -1) {
                ...
                EngineStdOut(
                    ...
                    "'. Defaulting to first costume if available.", 2);
                if (!objInfo.costumes.empty()) {
                    objInfo.selectedCostumeId = objInfo.costumes[0].id;
                }
            ...
            if (listeningEntity) {
                ...
                if (objInfoPtr) {
                    ...
                    EngineStdOut(
                        ...
                        "' processing. Script not run.", 1, executionThreadId);
                }
            } else {
                EngineStdOut(
                    "Entity " + listeningObjectId +
                    " not found when trying to execute message-received script for message '" + messageId + "'.", 1,
                    executionThreadId);
            }
        }
    } else {
...
void Engine::dispatchScriptForExecution(const std::string &entityId, const Script *scriptPtr,
    ...
    boost::asio::post(m_scriptThreadPool, [this, entityId, scriptPtr, sceneIdAtDispatch]() {
        ...
        try {
            ...
            std::stringstream ss;
            // std::thread::id를 직접 스트림에 출력하는 것은 표준적으로 지원되지 않을 수 있습니다.
            // 대신 해시 값을 사용하여 문자열로 변환합니다.
            ss << std::hash<std::thread::id>{}(current_thread_id);
