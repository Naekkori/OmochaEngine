...
bool Engine::loadProject(const string& projectFilePath, const string& projectDirectory) {
                ...
                if (objectJson.isMember("sprite") && objectJson["sprite"].isObject() &&
                    ...
                    for (const auto& pictureJson : picturesJson)
                    {
                        if (pictureJson.isObject() && pictureJson.isMember("id") && pictureJson["id"].isString() &&
                            pictureJson.isMember("filename") && pictureJson["filename"].isString()) { //엔트리 스프라이트 배열에는 assetId 가 없음.
                    ...
                    if (objectJson.isMember("entity") && objectJson["entity"].isObject()) {
                        ...
                        if (entityJson.isMember("font") && entityJson["font"].isString()) {
                            ...
                            if (pxPos != string::npos) {
                                ...
                                objInfo.fontSize = 20; objInfo.fontName = fontString;
                            }
                        }
                        else {
                            objInfo.fontSize = 20; objInfo.fontName = "";
                        }
                        ...
                        objInfo.textColor = GetColor(0, 0, 0); objInfo.fontName = ""; objInfo.fontSize = 20; objInfo.textAlign = 0;
                    }
                }
                else {
                    objInfo.textContent = ""; objInfo.textColor = GetColor(0, 0, 0); objInfo.fontName = ""; objInfo.fontSize = 20; objInfo.textAlign = 0;
                }
    ...
    if (!startSceneId.empty() && scenes.count(startSceneId)) {
        ...
        if (!firstSceneIdInOrder.empty() && scenes.count(firstSceneIdInOrder)) { // 첫 번째 씬 ID가 유효한지 확인
...
void Engine::drawAllEntities()
{
    ...
    for (int i = static_cast<int>(objects_in_order.size()) - 1; i >= 0; --i) { // vector 순회 (순서 유지)
        ...
        if (objInfo.objectType == "sprite") {
            ...
            if (selectedCostume && selectedCostume->imageHandle != -1) {
                double entryX = entityPtr->getX(); double entryY = entityPtr->getY();
                double regX = entityPtr->getRegX(); double regY = entityPtr->getRegY();
                double scaleX = entityPtr->getScaleX(); double scaleY = entityPtr->getScaleY();
