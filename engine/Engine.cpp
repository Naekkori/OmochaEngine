...
static void Helper_RenderFilledRoundedRect(SDL_Renderer *renderer, const SDL_FRect *rect, float radius)
{
    ...
    if (r > 0)
    {
        ...
        Helper_DrawFilledCircle(renderer, static_cast<int>(x + w - r), static_cast<int>(y + h - r), static_cast<int>(r));
    }
    else if (r == 0.0f)
    {
...
bool Engine::loadProject(const string &projectFilePath)
{
    ...
    if (document.HasMember("specialConfig"))
    {
        ...
        if (specialConfigJson.IsObject())
        {
            ...
            {
                this->specialConfig.setZoomfactor = clamp(specialConfigJson["setZoomfactor"].GetDouble(), (double)Engine::MIN_ZOOM, (double)Engine::MAX_ZOOM);
            }
            else
            {
                ...
                if (objectJson.HasMember("entity") && objectJson["entity"].IsObject())
                {
                    ...
                    if (entityJson.HasMember("textAlign") && entityJson["textAlign"].IsNumber())
                    {
                        ...
                        else
                        {
                            ...
                            EngineStdOut("textBox '" + objInfo.name + "' 'textAlign' field has an invalid value: " + to_string(parsedAlign) + ". Using default alignment 0 (left).", 1);
                        }
...
bool Engine::initGE(bool vsyncEnabled, bool attemptVulkan)
{
    ...
    { // 렌더러와 HUD 변수가 모두 유효할 때만 실행
        ...
        {
            EngineStdOut("Window dimensions (W:" + to_string(windowW) + ", H:" + to_string(windowH) + ") not valid for initial HUD clamping.", 1);
        }
    }
    return true;
}
