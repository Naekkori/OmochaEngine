        ...
        if (fieldType == "number" || fieldType == "text_reporter_number")
        {
            ...
            engine.EngineStdOut(
                "Invalid 'number' or 'text_reporter_number' block structure in parameter field for " + objectId +
                    ". Expected params[0] to be a string or number.",
                1, executionThreadId);
        ...
        else if (fieldType == "text" || fieldType == "text_reporter_string")
        {
            ...
            engine.EngineStdOut(
                "Invalid 'text' or 'text_reporter_string' block structure in parameter field for " + objectId +
                    ". Expected params[0] to be a string.",
                1, executionThreadId);
            return OperandValue("");
        }
                ...
                !paramField["params"].empty())
            {
                ...
                else if (param_zero.is_string())
                {
                        ...
                        if (is_number(angle_str))
                        {
                            ...
                            engine.EngineStdOut(
                                format(
                                    ...
                                    angle_str, angle_picker, objectId),
                                3, executionThreadId);
                            return OperandValue(angle_picker);
                        }
                        else
                        {
                            engine.EngineStdOut(
                                "Error: 'angle' block (object " + objectId +
                                    ") param is string but not a valid number: \"" + angle_str + "\". Params: " + paramField.dump() +
                                    " --- Returning NaN.",
                                2, executionThreadId);
                    ...
                    catch (const invalid_argument &ia)
                    {
                        engine.EngineStdOut(
                            "Error: 'angle' block (object " + objectId +
                                ") invalid argument for stod (string to double conversion): \"" + angle_str + "\". Params: " + paramField.dump() +
                        ...
                        const string &executionThreadId)
{
    if (BlockType == "calc_basic")
    {
        ...
        if (anOperator == "PLUS")
        {
            ...
            bool leftIsNumeric = (leftOp.type == OperandValue::Type::NUMBER || (leftOp.type == OperandValue::Type::STRING && !leftOp.string_val.empty() &&
                                                                                is_number(leftOp.string_val)));
            bool rightIsNumeric = (rightOp.type == OperandValue::Type::NUMBER || (rightOp.type == OperandValue::Type::STRING && !rightOp.string_val.empty() &&
        ...
        else
        {
            ...
            engine.EngineStdOut(
                "calc_rand block for object " + objectId + ": min_val (" + to_string(min_val) +
                    ") is greater than max_val (" + to_string(max_val) + "). Returning min_val.",
                1,
                executionThreadId);
        ...
        if (targetIdOpVal.type != OperandValue::Type::STRING)
        {
            engine.EngineStdOut(
                "coordinate_object block for " + objectId +
                    ": target object ID parameter (original index 1) did not resolve to a string. Value: " + targetIdOpVal.asString(),
                2,
                executionThreadId);
            return OperandValue(0.0);
        }
    ...
    else if (BlockType == "distance_something")
    {
        ...
        {
            engine.EngineStdOut(
                "distance_something block for " + objectId + ": target parameter is not a string. Value: " + targetIdOp.asString(), 2, executionThreadId);
            return OperandValue(0.0);
        }
    ...
    else if (BlockType == "change_hex_to_rgb")
    {
        ...
        {
            engine.EngineStdOut(
                "change_hex_to_rgb block for " + objectId + " has invalid params structure. Expected 2 params (HEX, CHANNEL).", 2,
                executionThreadId);
            return OperandValue(0.0);
        }
        OperandValue hexOp = getOperandValue(engine, objectId, block.paramsJson[0], executionThreadId);
        OperandValue channelOp = getOperandValue(engine, objectId, block.paramsJson[1], executionThreadId);

        if (hexOp.type != OperandValue::Type::STRING || channelOp.type != OperandValue::Type::STRING)
        {
            engine.EngineStdOut("change_hex_to_rgb block for " + objectId + " has non-string parameter(s).", 2,
                                executionThreadId);
            return OperandValue(0.0);
        }
        string hexStr = hexOp.string_val;
        string channel = channelOp.string_val;
        ...
        try
        {
            ...
            if (channel == "r")
            {
                return OperandValue(static_cast<double>(red));
            }
            else if (channel == "g")
            {
            ...
            {
                engine.EngineStdOut("change_hex_to_rgb block for " + objectId + " has unknown channel: " + channel, 2, executionThreadId);
                return OperandValue(0.0); // 알 수 없는 채널이면 0.0 반환
            }
        }
        catch (const invalid_argument &ia)
        {
        ...
        if (!block.paramsJson.is_array() || block.paramsJson.empty())
        {
            engine.EngineStdOut(
                "is_press_some_key block for " + objectId +
                    " has invalid or missing params array. Expected key identifier at index 0.",
                2, executionThreadId);
                    ...
                    if (idx != indexStr.length() || !isfinite(resolvedIndex_1based))
                    {
                        engine.EngineStdOut(
                            "value_of_index_from_list: INDEX string '" + indexStr +
                                "' is not a valid number for list '" + listIdToFind + "'.",
                            1, executionThreadId);
                        return OperandValue("");
                    }
                }
        ...
        if (finalIndex_1based < 1 || finalIndex_1based > static_cast<long long>(listArray.size()))
        {
            engine.EngineStdOut(
                "value_of_index_from_list: INDEX " + to_string(finalIndex_1based) + " is out of bounds for list '" +
                    listIdToFind + "' (size: " + to_string(listArray.size()) + ").",
                1, executionThreadId);
            return OperandValue("");
        }
        ...
        if (!block.paramsJson.is_array() || block.paramsJson.empty())
        {
            engine.EngineStdOut(
                "reach_something block for " + objectId +
                    " has invalid or missing params array. Expected target ID at index 0.",
                2, executionThreadId);
            return OperandValue(false);
        }
        ...
        if (!block.paramsJson.is_array() || block.paramsJson.size() < 3)
        {
            engine.EngineStdOut(
                "boolean_basic_operator block for " + objectId +
                    " has insufficient parameters. Expected LEFTHAND, OPERATOR, RIGHTHAND.",
                2, executionThreadId);
            return OperandValue(false);
        }
    ...
    else if (BlockType == "text_read")
    {
        ...
        {
            engine.EngineStdOut(
                "text_read block for " + objectId + ": target ID parameter is not a string. Value: " + targetIdOp.asString(), 2, executionThreadId);
            return OperandValue("");
        }
           ...
           const string &executionThreadId)
{
        ...
        else if (effectName == "brightness")
        {
            ...
            engine.EngineStdOut(
                "Entity " + objectId + " effect 'brightness' changed by " + to_string(value) + ", new value: " +
                    to_string(entity->getEffectBrightness()),
                3);
        }
        else if (effectName == "transparency")
        {
            ...
            engine.EngineStdOut(
                "Entity " + objectId + " effect 'transparency' (alpha) changed by " + to_string(value) +
                    "%, new value: " + to_string(entity->getEffectAlpha()),
                3);
        }
        else
        {
    ...
    else if (BlockType == "change_scale_size")
    {
        ...
        {
            engine.EngineStdOut(
                "change_scale_size block for " + objectId + ": VALUE parameter is not a number. Value: " + size.asString(), 2);
        }
    ...
    else if (BlockType == "set_scale_size")
    {
        ...
        {
            engine.EngineStdOut(
                "set_scale_size block for " + objectId + ": VALUE parameter is not a number. Value: " + setSize.asString(), 2);
        }
        ...
        if (!block.paramsJson.is_array() || block.paramsJson.size() < 1)
        {
            ...
            engine.EngineStdOut(
                "sound_volume_change block for object " + objectId +
                    " has insufficient parameters. Expected VALUE.",
                2,
                executionThreadId);
            return;
        }
              ...
              const string &executionThreadId)
{
    ...
    else if (BlockType == "add_value_to_list")
    {
        ...
        if (listIdOp.type != OperandValue::Type::STRING)
        {
            engine.EngineStdOut(
                "add_value_to_list block for " + objectId +
                    ": LIST_ID parameter (index 1) did not resolve to a string. Value: " + listIdOp.asString(),
                2,
                executionThreadId);
            return;
        }
    ...
    else if (BlockType == "remove_value_from_list")
    {
        ...
        if (listIdOp.type != OperandValue::Type::STRING)
        {
            engine.EngineStdOut(
                "remove_value_from_list block for " + objectId +
                    ": LIST_ID parameter (index 0) did not resolve to a string. Value: " + listIdOp.asString(),
                2,
                executionThreadId);
            return;
        }
        ...
        engine.EngineStdOut(
            "Removed item at index " + to_string(index_1_based) + " (value: '" + removedItemData + "') from list '" +
                listIdToFind + "' for object " + objectId,
            0, executionThreadId);
        if (targetListPtr->isCloud)
        {
            engine.saveCloudVariablesToJson();
          ...
          const string &executionThreadId, const string &sceneIdAtDispatch, float deltaTime)
{
    ...
    if (BlockType == "wait_second")
    {
        ...
        {
            engine.EngineStdOut(
                "Flow 'wait_second' for " + objectId + ": Time parameter is not a number. Value: " + secondsOp.asString(),
                2, executionThreadId);
            return;
        }
        ...
        engine.EngineStdOut(
            "Flow 'wait_second': " + objectId + " (Thread: " + executionThreadId + ") setting wait for " +
                to_string(secondsToWait) + "s. Script will pause.",
            3, executionThreadId);
        ...
        if (currentIteration < iterCount)
        {
            ...
            engine.EngineStdOut(
                "repeat_basic: " + objectId + " executing iteration " + to_string(currentIteration) + "/" +
                    to_string(iterCount) + ". Block ID: " + block.id,
                3, executionThreadId);
            ...
            if (pThreadState->isWaiting)
            {
                ...
                engine.EngineStdOut(
                    "Flow 'repeat_basic' for " + objectId + " pausing at iteration " + to_string(currentIteration) +
                        " because inner block set wait. Block ID: " + block.id,
                    3, executionThreadId);
                return;
            }
            ...
            if (innerBlockIsWaiting)
            {
                ...
                engine.EngineStdOut(
                    "Flow 'repeat_while_true' for " + objectId + ": Inner block set wait. Storing resume index " +
                        to_string(resumeIndexFromInnerExec) + ".",
                    3, executionThreadId);
            }
            else
            {
        ...
        else
        {
            engine.EngineStdOut(
                "Flow 'stop_repeat': ScriptThreadState not found for " + executionThreadId +
                    " or entity is null. Cannot request break.",
                2, executionThreadId);
        }
        ...
        else
        {
            engine.EngineStdOut(
                "Flow 'continue_repeat': ScriptThreadState not found for " + executionThreadId +
                    " or entity is null. Cannot request continue.",
                2, executionThreadId);
        }
        ...
        if (targetOptionOp.type != OperandValue::Type::STRING)
        {
            engine.EngineStdOut(
                "Flow 'stop_object' for " + objectId + ": TARGET parameter is not a string. Value: " +
                    targetOptionOp.asString() + ". Block ID: " + block.id,
                2, executionThreadId);
    ...
    else if (BlockType == "create_clone")
    {
        ...
        {
            engine.EngineStdOut(
                "Flow 'create_clone' for " + objectId + ": Target parameter is not a string. Value: " + targetOp.asString() + ". Block ID: " + block.id, 2, executionThreadId);
        ...
        {
            engine.EngineStdOut(
                "Error: Flow 'create_clone' - Could not determine a valid base object ID for cloning. Defaulting to current object ID: " + objectId, 2, executionThreadId);
           ...
           const string &executionThreadId)
{
    ...
    else if (BlockType == "start_neighbor_scene")
    {
        ...
        if (block.paramsJson.is_null())
        {
            engine.EngineStdOut(
                "start_neighbor_scene block for object " + objectId +
                    " has null paramsJson. Expected one parameter (next/prev).",
                2,
                executionThreadId);
            throw ScriptBlockExecutionError(
        ...
        {
            engine.EngineStdOut(
                "start_neighbor_scene block for object " + objectId + ": parameter is not a string. Value: " + o.asString(), 2, executionThreadId);
            return;
        }
                                ...
                                size_t start_index)
{
    engine.EngineStdOut(
        "Enter executeBlocksSynchronously for " + objectId + ". blocks.size() = " + to_string(blocks.size()) +
            ", start_index = " + to_string(start_index) + ". Thread: " + executionThreadId,
        3,
        executionThreadId);
    ...
    for (auto it = blocks.begin() + start_index; it != blocks.end(); ++it)
    {
        ...
        if (engine.m_isShuttingDown.load(memory_order_relaxed))
        {
            engine.EngineStdOut("executeBlocksSynchronously: Engine shutdown. Cancelling execution for entity " +
                                    objectId + ". Thread: " + executionThreadId,
                                1, executionThreadId);
            return;
        }
